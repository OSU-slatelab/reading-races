// This JS file is meant to run in a google chrome snippet on /addstory.html
// It will add all the stories in the stories variable
// Set up the stories in a format like this, or however you want, adjusting the code accordingly
...
const saveStoryButton = document.getElementById("addStoryButton");
...
function nextOne() {
  const currentKey = storyKeys[index];

  if (currentKey) {
    storyTitle.innerText = currentKey + "-" + stories[currentKey];
    storyContent.innerText = stories[currentKey];
    ...
    setTimeout(() => {
      index++;
      nextOne();
    }, 4000); // wait four seconds for the story to save in the database
  }
}
